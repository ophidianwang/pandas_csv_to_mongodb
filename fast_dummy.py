...
postfix = sys.argv[1]
log_file = open("fast_dummy_{0}.log".format(postfix), "a")
...
# for iter in xrange(50):  # put iter outside to simulate real loading time
for filename in file_list:
    ...
    df['RECORD_OPENING_TIME'] = mk_random_series(df, start_timestamp, end_timestamp)
    df = df[["SERVED_MSISDN", "TAC", "ECI", "LAC", "SAC", "SITE_TYPE"]]
    ...
    row_count = len(df_join_4g) + len(df_join_3g)

    # convert and async insert
    # df_dict = df_join.to_dict(orient="index")
    # collection.insert_many(df_dict.values(), ordered=False)
    for iter in xrange(10):  # put iter inside for faster dummy data insert 3host*3py*100*1,080,000 = 1,000,000,000
        # print("working on {1}, in loop #{0}".format(iter, filename))

        df_join_4g['SERVED_MSISDN'] = mk_random_id(df_join_4g)  # dummy msisdn
        df_join_4g['RECORD_OPENING_TIME'] = mk_random_series(df_join_4g, start_timestamp, end_timestamp)  # dummy time

        df_join_3g['SERVED_MSISDN'] = mk_random_id(df_join_3g)  # dummy msisdn
        df_join_3g['RECORD_OPENING_TIME'] = mk_random_series(df_join_3g, start_timestamp, end_timestamp)  # dummy time

        before_once = time()

        """
        for row in df_join_4g.to_dict(orient="index").values():
            print(row)
        for row in df_join_3g.to_dict(orient="index").values():
            print(row)
        """
        ...
        insert_per_sec = row_count / (after_once - before_once)

        log_file.write("spend {0} seconds on {1}, in loop #{2} @ {3}, insert {4} rows per second\n".format(
            after_once - before_once, filename, iter, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), insert_per_sec))
        print("spend {0} seconds on {1}, in loop #{2} @ {3}, insert {4} rows per second\n".format(
            after_once - before_once, filename, iter, datetime.now().strftime("%Y-%m-%d %H:%M:%S"), insert_per_sec))
